<script lang="ts">
	import { t } from '$lib/i18n';
	import LanguageSwitcher from '$lib/components/LanguageSwitcher.svelte';
</script>

<section class="relative min-h-[90vh] flex items-center overflow-hidden bg-[#0a0a0f]">
	<!-- Subtle grid -->
	<div class="absolute inset-0 opacity-[0.03]" style="background-image: url(&quot;data:image/svg+xml,%3Csvg width='60' height='60' xmlns='http://www.w3.org/2000/svg'%3E%3Crect width='60' height='60' fill='none' stroke='white' stroke-width='0.5'/%3E%3C/svg%3E&quot;)"></div>
	<!-- Glow orbs -->
	<div class="absolute top-1/4 -left-32 w-96 h-96 bg-violet-600/20 rounded-full blur-[120px]"></div>
	<div class="absolute bottom-1/4 -right-32 w-80 h-80 bg-blue-500/15 rounded-full blur-[100px]"></div>

	<!-- Language switcher -->
	<div class="absolute top-6 right-6 z-10">
		<LanguageSwitcher />
	</div>

	<div class="relative max-w-4xl mx-auto px-6 py-24 sm:py-32">
		<div class="flex flex-col items-center text-center">
			<!-- Badge -->
			<div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/10 bg-white/5 text-sm text-gray-400 mb-8 backdrop-blur-sm">
				<span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
				{$t.hero.badge}
			</div>

			<h1 class="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight text-white leading-[1.1]">
				{$t.hero.h1Line1}
				<span class="bg-gradient-to-r from-violet-400 via-blue-400 to-cyan-400 bg-clip-text text-transparent">{$t.hero.h1Line2}</span>
			</h1>

			<p class="mt-6 text-lg sm:text-xl text-gray-400 max-w-2xl leading-relaxed">
				{$t.hero.subtitle}
			</p>

			<div class="mt-10 flex flex-col sm:flex-row items-center gap-4">
				<a
					href="/signup"
					class="group relative px-8 py-3.5 rounded-xl font-medium text-white bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 transition-all duration-300 shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40"
				>
					{$t.hero.cta1}
				</a>
				<a
					href="https://github.com/polly3223/Rachel8"
					class="px-8 py-3.5 rounded-xl font-medium text-gray-300 border border-white/10 hover:border-white/25 hover:text-white transition-all duration-300"
					target="_blank"
					rel="noopener noreferrer"
				>
					{$t.hero.cta2}
				</a>
			</div>

			<p class="mt-5 text-sm text-gray-600">
				{$t.hero.subtext}
			</p>
		</div>
	</div>
</section>
